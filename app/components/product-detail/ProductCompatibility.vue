...
<template>
  <section id="tuong-thich" class="scroll-mt-28 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 lg:p-6">
    <h2 class="text-lg font-bold text-slate-900 lg:text-xl">Tương thích PC Builder</h2>
    ...
    <template v-else-if="data">
      ...
      <dl v-if="data.facts.length" class="mt-3 divide-y divide-slate-100 rounded-xl border border-slate-200 px-3"><div v-for="fact in data.facts" :key="fact.label" class="flex justify-between gap-4 py-2.5 text-sm"><dt class="text-slate-600">{{ fact.label }}</dt><dd class="font-medium text-slate-800">{{ fact.value }}</dd></div></dl>
