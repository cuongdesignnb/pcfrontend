...
<template>
  <section id="mo-ta" class="scroll-mt-28 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 lg:p-6">
    <h2 class="text-lg font-bold text-slate-900 lg:text-xl">Mô tả sản phẩm</h2>
    <div v-for="block in blocks" :key="block.id" class="mt-4">
      <template v-if="block.type === 'hero_banner'">
        ...
        <p v-if="text(block.payload, 'description')" class="mt-2 whitespace-pre-line leading-6 text-slate-700">{{ text(block.payload, 'description') }}</p>
      </template>
      ...
      <template v-else-if="block.type === 'image_text'">
        <div class="grid gap-4 md:grid-cols-2 md:items-center"><NuxtImg v-if="text(block.payload, 'image_url')" :src="text(block.payload, 'image_url')" :alt="text(block.payload, 'alt') || block.title || 'Hình ảnh sản phẩm'" class="w-full rounded-xl" /><div><h3 v-if="block.title" class="text-lg font-bold text-slate-900">{{ block.title }}</h3><p class="mt-2 whitespace-pre-line leading-6 text-slate-700">{{ text(block.payload, 'description') }}</p></div></div>
      </template>
    </div>
    <p v-if="description" class="mt-5 whitespace-pre-line leading-6 text-slate-700">{{ description }}</p>
    <p v-if="!description && !blocks.length" class="mt-3 text-sm text-slate-500">Sản phẩm chưa có mô tả chi tiết.</p>
  </section>
</template>
