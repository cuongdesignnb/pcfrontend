...
<template>
  <AccountShell>
    <div class="account-subpage-heading"><div><h1>Đơn hàng của tôi</h1><p>Theo dõi trạng thái và xem lại lịch sử mua hàng của bạn.</p></div><NuxtLink to="/danh-muc" class="account-dashboard-shop-link">Tiếp tục mua sắm <CartIcon name="arrow-right" size="16" /></NuxtLink></div>
    ...
    <section v-else class="account-list-card">
      <div v-if="!orders.length" class="account-empty-state"><CartIcon name="receipt" size="30" /><h3>Bạn chưa có đơn hàng nào</h3><p>Đơn hàng sau khi đặt sẽ được cập nhật tại đây.</p><NuxtLink to="/danh-muc" class="account-outline-button">Khám phá sản phẩm</NuxtLink></div>
