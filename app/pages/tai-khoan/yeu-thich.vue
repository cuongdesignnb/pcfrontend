...
<template>
  <AccountShell>
    <div class="account-subpage-heading"><div><h1>Sản phẩm yêu thích</h1><p>Những sản phẩm bạn đã lưu để xem lại hoặc mua sau.</p></div><NuxtLink to="/danh-muc" class="account-dashboard-shop-link">Khám phá sản phẩm <CartIcon name="arrow-right" size="16" /></NuxtLink></div>
    ...
    <section v-else class="account-list-card"><div v-if="!products.length" class="account-empty-state"><CartIcon name="heart" size="30" /><h3>Danh sách yêu thích đang trống</h3><p>Nhấn biểu tượng trái tim trên sản phẩm để lưu lại lựa chọn của bạn.</p><NuxtLink to="/danh-muc" class="account-outline-button">Khám phá sản phẩm</NuxtLink></div><div v-else class="account-full-wishlist-grid"><ProductCard v-for="product in products" :key="product.id" :product="product" variant="category" /></div></section>
  </AccountShell>
</template>
